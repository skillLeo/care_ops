...
class AttendanceController2026 extends Controller
{
    public function index()
    {
        ...
        return view('attendance2026.index', compact('submissions', 'submitters'));
    }
    ...
    public function batchOuting(Request $request)
    {
        ...
        return view('attendance2026.batch_outing', compact(
            'attendanceMeta',
    ...
    public function batchOutingStore(Request $request)
    {
        ...
        if (! $request->boolean('confirm')) {
            ...
            return view('attendance2026.batch_outing_review', [
                'rows' => $reviewRows,
                'meta' => $validated,
        ...
        foreach ($rows as $row) {
            Attendance2026::create([
                ...
                'remarks' => $row['remarks'] ?? $validated['remarks'],
                'marked_by' => auth()->id(),
        ...
        return redirect()
            ->route('attendance2026.index')
            ->with('success', 'Attendance 2026 batch outing updated.');
    }
    ...
    public function productivitySheet()
    {
        ...
        return view('attendance2026.prod_sheet', compact(
            'clients',
            'selectedDate',
            'serviceCode',
            'clientOptions',
    ...
    public function productivitySheetStore(Request $request)
    {
        ...
        if (! $request->boolean('confirm')) {
            ...
            return view('attendance2026.prod_sheet_review', [
                'rows' => $reviewRows,
                'meta' => $validated,
                'serviceCode' => $serviceCode,
                'attachments' => $attachments,
        ...
        return redirect()
            ->route('attendance2026.index')
            ->with('success', 'CPRS Productivity Sheet submitted.');
    }
    ...
    public function groupTherapy(Request $request)
    {
        ...
        $hasLevelFilter = (bool) $selectedLevel;
        $hasGroupFilter = $selectedGroupId && $selectedGroupId !== 'all';
        ...
        if ($selectedDate && $selectedLevel && $selectedGroupId) {
            $clientsQuery = Client::with(['apartment.house'])
                ->whereDate('starting_date', '<=', $selectedDate)
                ->where(function ($q) use ($selectedDate) {
                    $q->whereNull('discharge_date')
                        ->orWhereDate('discharge_date', '>=', $selectedDate);
            ...
            if ($hasLevelFilter || $hasGroupFilter) {
                $clientsQuery->whereHas('levelOfCareHistory', function ($query) use ($selectedDate, $selectedGroupId, $level, $hasLevelFilter, $hasGroupFilter) {
                    ...
                    if ($hasLevelFilter && $level) {
                        $query->where('level_of_care', $level->id);
                    }
                });
            }

            if ($hasGroupFilter) {
                $clientsQuery->whereHas('clientGroupHistory', function ($query) use ($selectedDate, $selectedGroupId) {
                    $query->whereDate('start_date', '<=', $selectedDate)
                        ->where(function ($query) use ($selectedDate) {
                            $query->whereNull('end_date')
                                ->orWhereDate('end_date', '>=', $selectedDate);
        ...
        return view('attendance2026.group_therapy', compact(
            'clients',
            'attendanceData',
            'selectedDate',
            'levels',
    ...
    public function groupTherapyStore(Request $request)
    {
        ...
        if (! $request->boolean('confirm')) {
            ...
            return view('attendance2026.group_therapy_review', [
    ...
    public function cprsGroup(Request $request)
    {
        ...
        return view('attendance2026.cprs_group', compact(
            'clients',
            'attendanceData',
            'attendanceMeta',
            'attendanceMetaTimeStart',
    ...
    public function cprsGroupStore(Request $request)
    {
        ...
        if (! $request->boolean('confirm')) {
            ...
            return view('attendance2026.cprs_group_review', [
                'presentClients' => $presentClients,
                'attendanceRows' => $attendanceRows,
                'meta' => $validated,
                'serviceCode' => $serviceCode,
    ...
    public function showSubmission(AttendanceSubmission $submission)
    {
        ...
        return view('attendance2026.submissions.show', compact('submission', 'attendanceRows'));
    }

    public function editSubmission(AttendanceSubmission $submission)
    {
        if ($submission->type !== 'group_therapy') {
            return view('attendance2026.submissions.edit', compact('submission'));
        }
        ...
        if ($selectedDate && $selectedLevel && $selectedGroupId) {
            ...
            $clientQuery->whereHas('levelOfCareHistory', function ($query) use ($selectedDate, $levelId) {
                ...
                if ($levelId) {
                    $query->where('level_of_care', $levelId);
                }
            });
        ...
        return view('attendance2026.submissions.edit', compact(
            'submission',
            'levels',
            'selectedLevel',
            'groups',
    ...
    public function updateSubmission(Request $request, AttendanceSubmission $submission)
    {
        ...
        return redirect()
            ->route('attendance2026.index')
            ->with('success', 'Attendance submission updated.');
    }
    ...
    public function destroySubmission(AttendanceSubmission $submission)
    {
        ...
        return redirect()
            ->route('attendance2026.index')
            ->with('success', 'Attendance submission deleted.');
    }
    ...
    public function downloadSubmission(AttendanceSubmission $submission)
    {
        ...
        $pdf = Pdf::loadView('attendance2026.submissions.pdf', [

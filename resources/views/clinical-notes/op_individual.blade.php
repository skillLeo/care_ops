...
@section('content')
    <div class="d-flex justify-content-start mb-3">
        <div class="btn-group" role="group" aria-label="Productivity sheet actions">
            ...
            <a href="{{ route('clinical-notes.op_individual') }}" class="btn btn-outline-secondary">Clear</a>
        </div>
    </div>
    @if (session('success'))
    ...
    <form method="POST" action="{{ route('clinical-notes.op_individual_store') }}" id="opIndividualForm" enctype="multipart/form-data">
        @csrf
        ...
        <div id="opIndividualAlert" class="alert alert-warning d-none" role="alert"></div>
        ...
        <div class="card mt-3">
            ...
            <div class="card-body p-0">
                <table class="table table-sm table-bordered mb-0" id="opIndividualTable">
                    <thead>
                        <tr>
                    ...
                    <tbody id="opIndividualBody"></tbody>
                </table>
            </div>
        </div>
    ...
    <div class="modal fade" id="opIndividualWarningModal" tabindex="-1" role="dialog" aria-labelledby="opIndividualWarningLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="opIndividualWarningLabel">Before you submit</h5>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-success" id="confirmOpIndividualSubmit">Submit</button>
                </div>
            </div>
        </div>
    </div>
...
@section('js')
    ...
    <script>
        ...
        const storedDraft = sessionStorage.getItem('opIndividualDraft');
        const storedMeta = storedDraft ? JSON.parse(storedDraft) : null;
        const draftRows = storedMeta?.rows ?? existingRows;
        const opIndividualAlert = document.getElementById('opIndividualAlert');
        const warningModalElement = document.getElementById('opIndividualWarningModal');
        ...
        function updateTotals() {
            const rows = document.querySelectorAll('#opIndividualBody tr');
            let totalClients = 0;
            let totalUnits = 0;
        ...
        function showFormAlert(message) {
            if (!opIndividualAlert) {
                return;
            }
            opIndividualAlert.textContent = message;
            opIndividualAlert.classList.remove('d-none');
        }
        ...
        function clearFormAlert() {
            ...
            opIndividualAlert.textContent = '';
            opIndividualAlert.classList.add('d-none');
        }
        ...
        function addRow(rowData = {}) {
            ...
            document.getElementById('opIndividualBody').appendChild(row);
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (storedMeta) {
                ...
                sessionStorage.removeItem('opIndividualDraft');
            }
            ...
            document.getElementById('opIndividualForm').addEventListener('submit', async function(event) {
                if (pendingSubmit) {
                    return;
                }
                const rows = document.querySelectorAll('#opIndividualBody tr');
        ...
        document.getElementById('confirmOpIndividualSubmit').addEventListener('click', () => {
            pendingSubmit = true;
            hideWarningModal();
            document.getElementById('opIndividualForm').submit();
        });

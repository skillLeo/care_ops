...
@section('title', 'Claims Calendar 2026 - ' . $client->last_name . ', ' . $client->first_name)

@section('content_header')
    <h1>Claim Submission Calendar (Attendance 2026) for {{ $client->last_name }}, {{ $client->first_name }}</h1>
@stop
...
@section('content')
    ...
    <div class="form-group d-flex align-items-center justify-content-between flex-wrap">
        ...
        <div class="d-flex align-items-center mt-2 mt-md-0 ml-md-4">
            ...
            <span class="badge badge-secondary">Checkboxes only show when attendance exists.</span>
            <span class="badge badge-success ml-1">Green text = unbilled.</span>
            <span class="badge badge-warning ml-1">Yellow text = billed but denied.</span>
            <span class="badge badge-secondary ml-1">Gray text = billed (not processed or denied).</span>
            <span class="badge badge-warning ml-1">Processed lines are hidden.</span>
        </div>
    </div>
    ...
    <div class="table-responsive">
        <table class="table table-bordered text-center">
            ...
            <tbody id="calendar-body" data-url="{{ route('clients.calendar2026.body', [$client->id]) }}">
...
@section('js')
    <script>
        ...
        let lineIndex = 0;
        const attendance2026 = @json($attendance2026);
        const processedClaims = @json($processedClaims);

        function getUnits(code, date) {
            return attendance2026?.[date]?.[code]?.units || 0;
        }

        function getPrice(code, date) {
            return attendance2026?.[date]?.[code]?.price ?? '';
        }

        function isProcessed(code, date) {
            return processedClaims?.[date]?.[code] === true;
        }
        ...
        function resolveDiagnosisCodeFinal(date) {
            ...
            if (uniqueAuthStarts.length !== 1) return '';
        ...
        function addLineFromCheckbox(date, code) {
            if (isProcessed(code, date)) return;

            const rate = getPrice(code, date);
            const units = getUnits(code, date);
            if (units <= 0) return;

            const billed = rate ? (rate * units).toFixed(2) : '';

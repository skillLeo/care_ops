...
@section('title', 'CPRS Productivity Sheet Review')

@section('content_header')
    <h1>CPRS Productivity Sheet Review</h1>
@stop
...
@section('content')
    ...
    <form method="POST" action="{{ route('attendance2026.prod_sheet_store') }}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mb-3">
            <label for="prod_sheet_review_attachments">Attachments</label>
            <input type="file" name="attachments[]" id="prod_sheet_review_attachments" class="form-control" multiple>
            @if (! empty($attachments))
...
@section('js')
    <script>
        ...
        document.getElementById('backToDraft').addEventListener('click', () => {
            ...
            const url = new URL(`{{ route('attendance2026.prod_sheet') }}`, window.location.origin);

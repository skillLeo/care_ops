...
@section('title', 'Attendance 2026 Submission')

@section('content_header')
    <h1>Attendance 2026 Submission</h1>
@stop
    ...
    <div class="card mb-3">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            @can('attendance2026.download')
                <a href="{{ route('attendance2026.submissions.download', $submission) }}" class="btn btn-sm btn-secondary">Download</a>
            @endcan
        </div>
        <div class="card-body">
            <div class="row">
                    ...
                    <div class="col-md-12 mb-2">
                        ...
                        <div class="d-flex flex-column gap-2">
                            @foreach ($submission->attachments as $file)
                                <a href="{{ route('attendance2026.submissions.attachments.download', [$submission, $file]) }}" class="btn btn-outline-info btn-sm">Download Attachment {{ $loop->iteration }}</a>
                            @endforeach
                        </div>
                    </div>
                </div>
                ...
                <tbody>
                    @foreach ($attendanceRows as $row)
                        <tr>
                            ...
                            <td class="text-center">{{ $row->serviceCode?->friendly_name }}</td>

...
@section('title', 'Peer Group Attendance Summary')

@section('content_header')
    <h1>Peer Group Attendance Summary</h1>
@stop

@section('content')
    <form method="GET" action="{{ route('reports.peerGroupAttendanceSummary') }}" class="mb-4">
            ...
            <div class="col-lg-2 col-md-3 mb-3">
                ...
                <select name="peer_group_id" id="peer_group_id" class="form-control">
                    ...
                    @foreach ($peerGroups as $peerGroup)
                        <option value="{{ $peerGroup->id }}" {{ (string) ($filters['peer_group_id'] ?? '') === (string) $peerGroup->id ? 'selected' : '' }}>{{ $peerGroup->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-lg-2 col-md-3 mb-3">

...
@section('title', 'Productivity - CPRS - Peer Outing Review')

@section('content_header')
    <h1>Productivity - CPRS - Peer Outing Review</h1>
@stop
...
@section('content')
    ...
    <form method="POST" action="{{ route('cprs.batch_outing_store') }}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mb-3">
            ...
            @else
                <div class="text-muted small">No attachments provided.</div>
            @endif
        </div>

        <button type="button" class="btn btn-secondary" id="backToEdit">Back</button>
    ...
    <script>
        document.getElementById('backToEdit').addEventListener('click', () => {
            ...
            sessionStorage.setItem('batchOutingDraft', JSON.stringify(meta));
            window.location.href = @json(route('cprs.batch_outing'));
        });
    </script>
@stop

...
@section('title', 'Edit Group Notes')

@section('content_header')
    <h1>Edit Group Notes for {{ $counselor->name }} ({{ $counselor->levelOfCare?->display_name ?? '-' }})</h1>
@stop

@section('content')
    <form action="{{ route('group_notes.update', $counselor->id) }}" method="POST" id="group-note-form">
        @csrf
        ...
        <input type="hidden" name="added_dates" id="added-dates">
        <input type="hidden" name="removed_dates" id="removed-dates">

        <button type="submit" class="btn btn-success mt-3">Update Group Notes</button>
    </form>
@stop
...
@section('js')
<script>
    let addedDates = {}; // Track added dates across months
    let removedDates = {}; // Track removed dates across months
    ...
    function generateCalendar(monthYear, selectedDates) {
        ...
        let currentAdded = addedDates[monthYear] || [];
        let currentRemoved = removedDates[monthYear] || [];
        ...
        for (let i = 0; i < 6; i++) {
            ...
            for (let j = 0; j < 7; j++) {
                ...
                if ((i === 0 && j < firstDay) || dateCounter > daysInMonth) {
                    ...
                    let checked = (selectedDates.includes(date) || currentAdded.includes(date)) && !currentRemoved.includes(date);

                    cell.innerHTML = `
                        <div style="${isCurrentDate} padding: 5px;">${dateCounter}</div>
                        <input type="checkbox" data-date="${date}" onchange="toggleDate(this, '${monthYear}')" ${checked ? 'checked' : ''} ${isPastDate} ${isRestricted ? 'hidden' : ''}>
                    `;
                    dateCounter++;
                }
    ...
    function toggleDate(checkbox, monthYear) {
        let date = checkbox.dataset.date;

        if (!addedDates[monthYear]) addedDates[monthYear] = [];
        if (!removedDates[monthYear]) removedDates[monthYear] = [];

        if (checkbox.checked) {
            addedDates[monthYear].push(date);
            removedDates[monthYear] = removedDates[monthYear].filter(d => d !== date);
        } else {
            removedDates[monthYear].push(date);
            addedDates[monthYear] = addedDates[monthYear].filter(d => d !== date);
        }
    }

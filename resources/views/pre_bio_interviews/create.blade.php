...
@section('title', 'Add Pre-Bio Interview')

@section('content_header')
    <h1>Add Pre-Bio Interview</h1>
@stop

@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('pre-bio-interviews.store') }}" method="POST">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <select name="client_id" class="form-control select2" required>
                        ...
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}">{{ $client->first_name }} {{ $client->last_name }} - MRN: {{ $client->mrn }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="ua_date" class="form-control">
                </div>

                <div class="mb-3">
                    <label class="form-label">Last UA Results</label>
                    <input type="text" name="ua_results" class="form-control">
                </div>
                ...
                @foreach($questions as $field => $data)
                    <div class="mb-3">
                        ...
                        <textarea name="{{ $field }}" class="form-control mt-2" rows="2"></textarea>
                    </div>
                @endforeach
